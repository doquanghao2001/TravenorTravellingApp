package com.example.travenortravellingapp.presentation.util

import androidx.compose.ui.unit.dp
import androidx.compose.ui.unit.sp

object Dimes {
    val LargeCornerRadius = 50.dp
    val MediumCornerRadius = 30.dp
    val SmallCornerRadius = 24.dp

    val SmallTextSize = 20.sp
    val MediumTextSize = 26.sp
    val LargeTextSize = 40.sp
    val ContentTextSize = 16.sp

    val VerySmallPadding = 8.dp
    val SmallPadding = 16.dp
    val MediumPadding = 24.dp
    val LargePadding = 48.dp

    val VerySmallLineHeight = 6.sp
    val SmallLineHeight = 12.sp
    val MediumLineHeight = 22.sp
    val LargeLineHeight = 44.sp


    val VerySmallBoxHeight = 18.dp
    val SmallBoxHeight = 32.dp
    val MediumBoxHeight = 56.dp
    val LargeBoxHeight = 84.dp


    val VerySmallBorderWidth = 6.dp
    val SmallBorderWidth = 12.dp
    val MediumBorderWidth = 24.dp
    val LargeBorderWidth = 48.dp
}